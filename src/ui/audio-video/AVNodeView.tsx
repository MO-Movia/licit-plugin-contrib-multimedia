...
import resolveVideo, { AVResult } from './ResolveAV';
...
export class VideoViewBody extends React.PureComponent {
  ...
  render(): React.ReactElement {
    ...
    const { attrs } = node;
    const { align, crop, rotate, isAudio } = attrs;
  ...
  _resolveOriginalSize = async (): Promise<void> => {
    ...
    let originalSize: AVResult;
    if (isRevokeNeeded) {
      URL.revokeObjectURL(this.props.node.attrs.src);
      const id = (this.props.node.attrs as AVEditorState).id;
      src = (this.props.node.attrs as AVEditorState).src;
      if (id !== '') {
        src = await this.props.editorView.runtime.getVideoSrc(id);
      }
      const props = this.props.node.attrs as AVEditorState;
      props.src = src;
      ...
      originalSize = await resolveVideo(
        this.props.node.attrs as AVEditorState
      );
    }
    if (!this._mounted) {
      // unmounted;
...
class VideoNodeView extends CustomNodeView {
  // @override
